from data.data_node import Position, Span, CompositeNode
...
class StructureAnalyzer:
    ...
    @classmethod
    def add_node_info(cls, doc, root, errors):
        """Add border informations about nodes"""
        lines = doc.splitlines()
        if isinstance(root,  CompositeNode):
            cls._analyze_node(lines,root, errors)
    
    @classmethod
    def _analyze_node(cls, lines, node, errors):
        """Recursivly is processed node analyzation"""
        ...
        if node_type == NodeStructureType.json_array or \
            ...
            if len(node.children) > 0 and \
                ...
                node.children[0].separators.start.line < node.children[len(node.children) - 1].separators.end.line:
                errors. report_multiline_flow(node)
        ...
        for child in node.children:
            if isinstance(child,  CompositeNode):
                cls._analyze_node(lines, child, errors)
        
    @classmethod
    def _split_children(cls, lines, node, func):
        """Find and write separators (borders) possitions between nodes"""
