...
class Transformator:
    ...
    def __init__(self,  transform_file, data = None):
        ...
        for action in self._transformation['actions']:
            ...
            if not action['action'] in ["delete-key", "move-key",  "rename-type", "move-key-forward"]:
    ...
    def _move_key_forward(self, root, lines, action):
        """Move key forward"""
        try:
            parent = re.search('^(.*)/([^/]*)$',action['parameters']['path'])
            node = root.get_node_at_path(action['parameters']['path'])
            if parent is None:
                raise TransformationFileFormatError("Cannot find parent path for path (" + 
        ...
        if not isinstance(parent_node,  dn.CompositeNode):
            raise TransformationFileFormatError("Parent of path (" + 
    ...
    def _delete_key(self, root, lines,  action):
        ...
        for i in range(l1, l2+1):
            if l1 == l2:
               ...
               text = lines[i]
            anchor = re.search('\s+&\s*(\S+)\s+', text)
            if anchor is None:
                anchor = re.search('\s+&\s*(\S+)$', text)
        ...
        for char in ["!", '\*','<<:\s*\*' ,"&"]:
            ...
            if  lines[nl1][nc1:].isspace() or len(lines[nl1]) <= nc1:
                ...
                while  lines[nl1].isspace() or len(lines[nl1]) == 0:
                    ...
                    if nl1 > l2:
                        return l1, c1
            tag = re.search('^(\s*' + char + '\S*)', lines[nl1][nc1:])
            if tag is not None:
                nc1 += len(tag.group(1))
    ...
    def _rewrite_first_ref(self, root, lines, anchors):
        ...
        refs_remove = []
        for r in refs:
            if  r in anchors and refs[r]['anchor'] is not None and \
                len(refs[r]['ref'])>0:
    ...
    def _copy_tree_from_anchor(self, lines, anchor_node, ref_node, remove_anchor=False):
        """Copy tree structure from anchor node to node with refference"""
        ...
        while dl1 <= dl2:
            ref =  re.search('^(.*\*' + anchor_node.anchor.value + ')(.*)$', lines[dl1])
            if ref is not None:
                anchor = "&" + anchor_node.anchor.value
                if remove_anchor:
                    anchor = ""
                lines[dl1] = re.sub( '\*' + anchor_node.anchor.value + "\s+", anchor + " ", lines[dl1])
                lines[dl1] = re.sub( '\*' + anchor_node.anchor.value + "$", anchor, lines[dl1])
    ...
    def _copy_value(self, lines, l1, c1, l2, c2, intend):
        ...
        if l1 == l2:
            ...
            if c1 > 0:
                ...
                from_line += 1
            intendation2 = re.search('^(\s*)(\S.*)$', lines[l1])
            ...
            intendation_test = re.search('^(\s*)(\S.*)$', lines[l2])
