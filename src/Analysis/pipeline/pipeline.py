...
class Pipeline(WorkflowActionType):
    ...
    def get_output(self, action, number):
        ...
        if 'OutputActions' in self.variables and \
            isinstance(self.variables['OutputActions'], list) and \
            len(self.variables['OutputAction'])<=number:
            return None
        if isinstance(self.variables['OutputActions'][number],  BaseActionType):
            return self.variables['OutputActions'].get_output(self, number)
        return None
    ...
    def _check_params(self):    
        ...
        if  not 'OutputActions' in self.variables:
            err.append("Parameter 'OutputActions' is require")
        elif isinstance(self.variables['OutputActions'], list):
            err.append("Parameter 'OutputActions' must be list of output actions")
        elif len(self.variables['OutputActions'], list)<1:
            err.append("Parameter 'OutputActions' must contains least one action")
        else:
            for i in range(0, len(self.variables['OutputActions'], list)):
                if not isinstance(self.variables['OutputActions'],  BaseActionType):
                    err.append("Type of parameter 'OutputActions[{0}]'  must be BaseActionType".format(str(i)))                    
        return err
    ...
    def _get_child_list(self):
        ...
        if 'OutputActions' in self.variables and \
            isinstance(self.variables['OutputActions'], list) and \
            len(self.variables['OutputActions'])>0 and \
            isinstance(self.variables['OutputActions'][0],  BaseActionType):            
            actions = self._get_action_list(self.variables['OutputActions'][0])
            for i in range(1, len(self.variables['OutputActions'])):
                if isinstance(self.variables['OutputActions'][i], BaseActionType):
                    actions2 = self._get_action_list(self.variables['OutputActions'][i])
    ...
    def _get_variables_script(self):
        ...
        if 'OutputActions' in self.variables and \
            isinstance(self.variables['OutputActions'], list) and \
            len(self.variables['OutputActions'])>0:
            array = 'OutputActions=['
            first = True
            for out in self.variables['OutputActions']:
