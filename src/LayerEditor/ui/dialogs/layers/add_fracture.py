...
class AddFractureDlg(QtWidgets.QDialog):
    ...
    def __init__(self, fracture_positions, parent=None):
        ...
        self.image.setPixmap(QtGui.QIcon("../common/icon/24x24/sign-check.png").pixmap(self.fracture_name.sizeHint().height()))
    ...
    def frac_name_changed(self, name):
        ...
        if self.is_unique_frac_name(name):
            self.image.setPixmap(
                QtGui.QIcon("../common/icon/24x24/sign-check.png").pixmap(self.fracture_name.sizeHint().height())
            )
            self.image.setToolTip('Unique name is OK.')
            self._tranform_button.setEnabled(True)
        else:
            self.image.setPixmap(
                QtGui.QIcon("../common/icon/24x24/warning.png").pixmap(self.fracture_name.sizeHint().height())
            )
