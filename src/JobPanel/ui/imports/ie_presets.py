    ...
    def import_(self, temp_item):
        ...
        for key in new_datas:
            ...
            for attr in self.items:
                if attr in new_data:
                    setattr(data, new_data[attr])
                elif(attr in self.default_items):
                    setattr(data, self.default_items[attr])
                else:
...
class IESsh(IE):
    items = ["host", "name", "pbs_system", "port", "remote_dir", "uid"]
    default_items = {"key":"", "pwd":"", "to_pc": False, "to_remote": False}
        
class IERes(IE):
    items = ["j_env", "j_execution_type", "j_pbs_preset", "j_remote_execution_type", 
        "j_ssh_preset", "mj_env", "mj_execution_type", "mj_pbs_preset", 
