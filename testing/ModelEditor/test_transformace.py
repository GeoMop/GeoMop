...
transform_test_data/05_flow_river.con
...
def test_transf():
    ...
    for i in range(0, len(input_files)):
        ...
        assert len(templ) == len(doc), \
            "Len of template {0} and result is diferent {1}!={2}.".format(
            output, len(templ),  len(doc))
        for i in range(0, len(doc)):
            assert templ[i].rstrip() == doc[i].rstrip(), \
                "Difference in line {0} of {1} (template, result)\n{2}\n<>\n{3}".format(
                str(i+1), output, templ[i].rstrip(), doc[i].rstrip())
